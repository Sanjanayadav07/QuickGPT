...
const SideBar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  const deleteChat = async (e, chatId) => {
    try {
      e.stopPropagation()
      const confirm = window.confirm('Are you want to delete this chat ?')
      if(!confirm) return
  ...
  return (
    <div className={`fixed top-0 left-0 z-1 flex flex-col h-screen min-w-72 p-5 
      dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 
      border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 
      max-md:absolute ${!isMenuOpen && 'max-md:translate-x-full'}`}>
      ...
      {/* New Chat Button */}
      <button onClick={createNewChat} className="flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] text-sm rounded-md cursor-pointer">
      ...
      onClick={() => {navigate('/community'); setIsMenuOpen(false)}}
        //onClick={() => {navigate('/community');isMenuOpen(false)}}
